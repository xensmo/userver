...
FutureStatus WaitAllCheckedContext::Impl::WaitUntil(Deadline deadline) {
    ...
    if (pending_notifications_.load(std::memory_order_acquire) > 0) {
        if (!result_ready_.WaitForEventUntil(deadline)) {
    ...
    if (exception_source != nullptr) {
        exception_source->RethrowErrorResult();
...
void WaitAllCheckedContext::Impl::DoNotify(std::uintptr_t context) noexcept {
    ...
    bool ready = false;
    try {
        targets_[index]->RethrowErrorResult();
    } catch (std::exception&) {
        ContextAccessor* expected{nullptr};
        exception_source_.compare_exchange_strong(expected, targets_[index], std::memory_order_relaxed);
        ready = true;
    }
