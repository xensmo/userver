...
void form_mime::add_content(std::string_view key, std::string_view content) {
    std::error_code ec;
    add_content(key, content, ec);
    throw(ec, "add_content");
}

void form_mime::add_content(std::string_view key, std::string_view content, std::error_code& ec) {
...
void form_mime::add_content(std::string_view key, std::string_view content, const std::string& content_type) {
    std::error_code ec;
    add_content(key, content, content_type, ec);
    throw(ec, "add_content");
}

void form_mime::add_content(std::string_view key, std::string_view content, const std::string& content_type, std::error_code& ec) {
    part_ = native::curl_mime_addpart(mime_);
    ec = std::error_code { static_cast<errc::EasyErrorCode>(native::curl_mime_data(part_, key.data(), key.size())) };
...
void form_mime::add_buffer(std::string_view key, std::string_view file_name, 
