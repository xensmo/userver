...
namespace detail {
    ...
    [[nodiscard]] inline constexpr long to_integral(_Et val) {
...
easy::easy(native::CURL* easy, native::curl_mime* mime, multi* multi) :
...
bool easy::has_post_data() const {
    return !post_fields_.empty() || form_mime_;
}
...
void easy::add_header(std::string_view name, std::string_view value, std::error_code& ec, 
    detail_eha eha = detail_eha::kSend, detail_dha dha = detail_dha::kAdd) {
...
void easy::set_share(std::shared_ptr<share> share) {
    ...
    if (share) {
        std::error_code ec = detail::set_curl_opt(easy_handle_, native::CURLOPT_SHARE, share->native_handle());
        throw(ec, "set_share");
    } else {
        std::error_code ec = detail::set_curl_opt(easy_handle_, native::CURLOPT_SHARE, NULL);
        throw(ec, "set_share");
    }
}
...
void easy::set_url(std::string_view url) {
    detail::set_curl_opt(
        "set_url", easy_handle_, native::CURLOPT_URL, url.data());
}
...
void easy::set_headers(std::shared_ptr<string_list> headers) {
    ...
    if (headers_)
        detail::set_curl_opt(
            "set_headers", easy_handle_, native::CURLOPT_HTTPHEADER, headers_->native_handle());
    else
        detail::set_curl_opt(
            "set_headers", easy_handle_, native::CURLOPT_HTTPHEADER, NULL);
}
...
void easy::set_time_value(native::curl_off_t value) {
    detail::set_curl_opt(
        "set_time_value", easy_handle_, native::CURLOPT_TIMEVALUE_LARGE, value);
}
...
void easy::set_resolves(std::shared_ptr<string_list> resolved_hosts) {
    ...
    if (resolved_hosts_) {
        std::error_code ec = detail::set_curl_opt(
            easy_handle_, native::CURLOPT_RESOLVE, resolved_hosts_->native_handle());
        throw(ec, "set_resolves native handle failed");
    } else {
        std::error_code ec = detail::set_curl_opt(
            easy_handle_, native::CURLOPT_RESOLVE, NULL);
        throw(ec, "set_resolves NULL failed");
    }
}
...
void easy::set_ssl_ctx_function(ssl_ctx_function_t func) {
    detail::set_curl_opt(
        "set_ssl_ctx_function", easy_handle_, native::CURLOPT_SSL_CTX_FUNCTION, func);
}

void easy::set_ssl_ctx_data(void* ptr) {
    detail::set_curl_opt(
        "set_ssl_ctx_data", easy_handle_, native::CURLOPT_SSL_CTX_DATA, ptr);
}
